...
在确认导入前，两种脚本都会先读取目标数据库的默认字符集和排序规则：

- 目标数据库不存在：确认后会按 `utf8mb4` 创建；
- 目标数据库已存在且不是 `utf8mb4`：会提示导入中文内容可能乱码，只有明确输入 **`YES`** 才会继续；输入其他内容会取消，且不会创建数据库或执行迁移；
- 字符集检查失败：脚本会停止，不会继续执行迁移。

这里检查的是目标数据库本身的默认字符集，不是 MySQL 客户端连接字符集；客户端虽然会按 `utf8mb4` 发送 SQL，但不能自动改变已有数据库的默认字符集。

...
`agent_max_toolcall_timeout`，消除后端实际超时与前端 120 秒提示不一致的问题。

## 当前问题

- `sub_agent_call` 当前使用独立的 `sub_agent_delegation_timeout_seconds`，默认 120 秒。
- `markStalePendingStreamLogs` 在前端固定使用 `120_000` 毫秒。
...
`agent_max_toolcall_timeout`。该读取保持现有默认值、合法范围和 Redis/数据库回退行为。
...
如果历史消息、旧服务端或异常流没有携带快照，前端保留 120 秒兼容兜底；该兜底只用于兼容显示，不改变后端任务执行，也不再作为正常新请求的配置来源。
...
- 后端配置读取失败或配置非法时，复用既有解析逻辑的安全默认值 120 秒。
- 前端收到非正数、非有限数或缺少字段时，忽略该快照并使用 120 秒兼容兜底。
...
- 验证看门狗使用消息配置值，而不是固定 120 秒。
- 验证缺少或非法快照时使用 120 秒兼容兜底。

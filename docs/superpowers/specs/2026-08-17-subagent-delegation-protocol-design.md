...
本次协议增强分为运行级关联和独立子会话命名空间两个层次。不引入独立持久化子会话、后台续聊、冷恢复或新的子代理后端；子代理仍由当前 Dispatcher/Executor 执行，但每次被接受的委派拥有明确的运行标识、父子 Trace 关系和子会话 ID。

## 当前基础

NanZi 已有 `SubAgentRequest`、`SubAgentResult`、目标智能体权限检查、就绪校验、单层深度限制、120 秒超时、重复委派限制、权限/外部执行中断识别和结果截断。

当前子代理使用独立的 `AgentContext`。第一阶段将子 Executor 的 `conversation_id` 切换为本次委派生成的 `child_session_id`，并将父会话保存为 `parent_conversation_id`；因此会话级状态读写不再隐式复用父会话命名空间。子代理仍只接收显式委派 query，不接收父会话历史。

为保持主聊天的实时展示和证据回流，`trace_buffer`、`event_queue` 和 grounding 账本仍由父子执行共享。这些共享对象只负责本轮运行的实时聚合，不改变子会话的状态隔离边界。
...
- `parent_conversation_id`：发起委派的父会话 ID。
- `child_session_id`：本次委派使用的独立子会话命名空间；目标 Executor 创建后由系统写入。
...
- `run_id`、`parent_trace_id`、`parent_conversation_id`、`child_trace_id`、`child_session_id`；
...
1. 生成 `run_id`，记录父 Trace；目标检查、工具过滤和重复委派检查通过后生成唯一 `child_session_id`。
...
现有共享 Trace buffer 继续保证主聊天的实时日志不丢失；`run_id`、父子 Trace 和父子会话字段用于检索和区分。`child_session_id` 是会话级服务的隔离命名空间，不表示本阶段已经具备独立 Session 持久化、子代理 transcript 或冷恢复能力。
...
页面展示不新增数据库表或会话恢复协议。没有子代理元数据的历史步骤继续按旧格式渲染；已有思考卡和轨迹仍从共享实时流中展示子代理步骤。

## 不做的事情

- 不改主助手的路由和委派触发策略。
- 不新增独立会话表、子代理 transcript 或 Session 恢复协议；本阶段只提供运行期的子会话命名空间。
- 不让子代理获得父代理未配置的工具。
...
- 每次通过目标、工具过滤和重复检查的委派产生唯一 `child_session_id`，且与父 `conversation_id` 不同。
- 子 Executor 的会话级读写使用 `child_session_id`；父会话历史不会通过 `conversation_id` 隐式注入。
- 结果和生命周期元数据包含 `parent_conversation_id` 与 `child_session_id`。

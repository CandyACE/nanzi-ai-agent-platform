...
| `llm_model_name` | `deepseek-chat` | 改为您在【模型管理】中配置的真实主对话模型名称（如 `deepseek-v3` / `gpt-4o` / `qwen-2.5-72b`） | • **影响平台兜底与系统级推理**：当会话未显式指定模型，或系统进行后台异步意图路由分类、TODO 任务拆解、上下文摘要压缩时，统一调用此全局兜底模型。 |
...
- **会话粘性恢复 (Session Sticky Routing) 与取消硬拦截**：
  - 用户提交选项或点击确认后，系统自动激活**会话粘性机制**，直接将用户决策注入上下文继续执行，**无需重新经过路由大模型判断**，大幅降低响应延迟；
...
- **解答**：
  ...
  - **防死循环硬拦截**：系统具备路由粘性与确认阻断机制，明确向模型表明该操作已被人类明确终止，模型会立刻收拢流程并转入后续解释或替代方案，绝不会再次无休止弹出同一张确认卡。
...
sequenceDiagram
    ...
    participant Decision as 🧭 单轨路由决策器 (TurnDecision)
    ...
    Master->>Decision: 单轨意图与上下文分析 (一次 LLM 周期完成路由+工具预检)
    Decision-->>Master: 输出执行计划：需要 2 位专家并行分析 + 1 位专家代码交付
...
1. **单轨决策器 (TurnDecision)**：
   - 传统多 Agent 系统往往需要先调一次意图模型判断去向，再调一次路由模型分发，延迟极高；平台收敛为 `TurnDecision` 单轨决策器，在单一推理周期内完成“意图识别 + 专家路由 + 动态工具装配”，调度开销降低 60% 以上；
...
##### Q6: 为什么我自己新建的自定义智能体，在「自动路由 (Auto-route)」模式下不会被自动分发到？

- **原因**：
  - 平台为了防止个人随意创建的实验性/草稿智能体过多，导致全局语义分发产生混乱与幻觉冲突，**只有被标记为“系统智能体”（`is_system` 系统级预置专家）的智能体，才会进入到全局自动路由调度器（Auto-route Dispatcher）的候选池中**；
- **如何使用与唤起自定义智能体**：
  1. **方式一（显式 `@` 呼出直连）**：在聊天输入框中输入 `@`，在下拉列表中直接点选该智能体，本轮对话将 100% 精准直达该专家；
  2. **方式二（设为当前会话默认智能体）**：在聊天页面打开「⚙️ 界面设置」，将默认主责智能体切换为您的自定义智能体；
  3. **方式三（升级为系统智能体）**：若该智能体能力成熟稳定，且需面向全平台提供全局自动化意图路由服务，可由管理员在后台将其属性配置为系统智能体。

##### Q7: 自动路由 (Auto-route) 到底是如何判断把用户问题分发给哪位专家的？

- **核心判定机制与依据**：
  平台意图路由调度引擎（Router / Dispatcher）在接收到用户的自然语言提问后，会提取候选系统智能体的核心元数据进行语义理解与意图匹配：
...
- **决策输出**：
  - 若命中某位专家的专长领域且置信度明确，系统自动委派该专家并执行专属任务流；
  - 若属于泛化提问或未命中任何垂直专长，则平滑兜底交给默认主助手（Main Agent）直接解答。
...
3. **哪些特殊配置需要重启服务？**：
   ...
   - **底层主数据库连接串** 或 **Redis Stack 端口变更**（由于涉及全局连接池单例与常驻调度器任务，需在修改 `.env` 或配置后重启 `./dev.sh`）。

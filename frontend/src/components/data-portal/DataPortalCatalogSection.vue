<template>
  <section>
    ...
    <div class="mt-3 divide-y divide-gray-100 border-y border-gray-100 dark:divide-gray-800 dark:border-gray-800">
      <article v-for="dataset in visibleDatasets" :key="dataset.key" class="py-3" :class="{ 'opacity-75': dataset.disabled }">
        <button type="button" class="flex w-full items-center justify-between gap-3 text-left" @click="toggleDataset(dataset.key)">
          <span><strong class="text-sm text-gray-800 dark:text-gray-200">{{ dataset.name }}</strong><span v-if="isDatasetDisabled(dataset)" class="ml-2 rounded border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-[10px] font-bold text-amber-700 dark:border-amber-800/60 dark:bg-amber-950/40 dark:text-amber-300">未启用</span><span class="ml-2 text-xs text-gray-400">{{ dataset.tables.length }} 张表 · {{ dataset.columnCount }} 个字段</span></span>
          <span class="text-xs text-blue-500">{{ compact ? '查看结构 →' : (expandedDatasets.has(dataset.key) ? '收起' : '展开') }}</span>
        </button>
        <div v-if="!compact && expandedDatasets.has(dataset.key)" class="mt-3 space-y-2 pl-0 sm:pl-3" :class="{ 'pointer-events-none select-none': isDatasetDisabled(dataset) }">
          <div v-for="table in dataset.tables" :key="table.name" class="rounded-xl border border-gray-100 bg-gray-50/70 dark:border-gray-800 dark:bg-gray-950/40">
            ...
            <div v-if="expandedTables.has(tableKey(dataset.key, table.name))" class="border-t border-gray-100 px-3 py-3 dark:border-gray-800">
              ...
              <div class="mt-3 flex flex-wrap gap-2">
                <button type="button" class="rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs text-blue-600 hover:bg-blue-100 disabled:cursor-not-allowed disabled:opacity-50" :disabled="isDatasetDisabled(dataset)" @click.stop="askTable(dataset, table, 'structure')">结构说明</button>
                <button type="button" class="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-xs text-emerald-600 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-50" :disabled="isDatasetDisabled(dataset)" @click.stop="askTable(dataset, table, 'query')">查询明细</button>
                <button type="button" class="rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs text-amber-600 hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50" :disabled="isDatasetDisabled(dataset)" @click.stop="askTable(dataset, table, 'recommend')">推荐提问</button>
              </div>
            </div>
          </div>
        </div>
...
interface CatalogDataset { key: string; name: string; tables: CatalogTable[]; columnCount: number; disabled: boolean }
...
const datasets = computed<CatalogDataset[]>(() => {
  const map = new Map<string, { name: string; tables: Map<string, CatalogTable>; disabled: boolean }>();
  for (const group of props.payload.groups || []) for (const related of group.related_data || []) {
    ...
    const disabled = group.enabled === false || related.enabled === false;
    const entry = map.get(key) || { name: related.display_name || related.dataset || "未命名数据集", tables: new Map(), disabled: false };
    entry.disabled = entry.disabled || disabled;
  ...
  return Array.from(map.entries()).map(([key, item]) => { const tables = Array.from(item.tables.values()); return { key, name: item.name, tables, columnCount: tables.reduce((sum, table) => sum + table.columns.length, 0), disabled: item.disabled }; });
});
...
const isDatasetDisabled = (dataset: CatalogDataset) => dataset.disabled;
...
const askTable = (dataset: CatalogDataset, table: CatalogTable, type: "structure" | "query" | "recommend") => {
  if (isDatasetDisabled(dataset)) return;
  const target = table.physicalName ? `“${table.name}”（物理表：${table.physicalName}）` : `“${table.name}”`;
  const query = type === "structure" ? `说明${dataset.name}数据集下数据表${target}的字段结构和分析口径` : type === "query" ? `查询数据表${target}最近10条明细数据` : `基于${dataset.name}数据集的数据表${target}，推荐适合业务分析的问题`;
